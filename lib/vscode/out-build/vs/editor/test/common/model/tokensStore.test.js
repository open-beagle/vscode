 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/editor/common/model/tokensStore", "vs/editor/common/core/range", "vs/editor/common/modes", "vs/editor/test/common/editorTestUtils", "vs/editor/common/core/lineTokens"], function (require, exports, assert, tokensStore_1, range_1, modes_1, editorTestUtils_1, lineTokens_1) {
    "use strict";
    ...
    suite('TokensStore', () => {
        ...
        function parseTokensState(state) {
            ...
            for (let i = 0; i < state.length; i++) {
                ...
                while (true) {
                    ...
                    const metadata = (SEMANTIC_COLOR << 14 /* FOREGROUND_OFFSET */
                        | 8 /* SEMANTIC_USE_FOREGROUND */);
                    if (tokens.length === 0) {
                        baseLine = i + 1;
                    }
            ...
            return {
                text: text.join('\n'),
                tokens: new tokensStore_1.MultilineTokens2(baseLine, new tokensStore_1.SparseEncodedTokens(new Uint32Array(tokens)))
            };
        }
        function extractState(model) {
            ...
            for (let lineNumber = 1; lineNumber <= model.getLineCount(); lineNumber++) {
                ...
                for (let i = 0; i < lineTokens.getCount(); i++) {
                    ...
                    const color = modes_1.TokenMetadata.getForeground(metadata);
        ...
        function testTokensAdjustment(rawInitialState, edits, rawFinalState) {
            ...
            const model = (0, editorTestUtils_1.createTextModel)(initialState.text);
        ...
        test('issue #91936: Semantic token color highlighting fails on line with selected text', () => {
            const model = (0, editorTestUtils_1.createTextModel)('                    else if ($s = 08) then \'\\b\'');
            model.setSemanticTokens([
                new tokensStore_1.MultilineTokens2(1, new tokensStore_1.SparseEncodedTokens(new Uint32Array([
                    0, 20, 24, 245768,
                    0, 25, 27, 245768,
                    0, 28, 29, 16392,
                    0, 29, 31, 262152,
        ...
        test('partial tokens 1', () => {
            const store = new tokensStore_1.TokensStore2();
            // setPartial: [1,1 -> 31,2], [(5,5-10),(10,5-10),(15,5-10),(20,5-10),(25,5-10),(30,5-10)]
            store.setPartial(new range_1.Range(1, 1, 31, 2), [
                new tokensStore_1.MultilineTokens2(5, new tokensStore_1.SparseEncodedTokens(new Uint32Array([
                    0, 5, 10, 1,
                    5, 5, 10, 2,
                    10, 5, 10, 3,
                    15, 5, 10, 4,
            ...
            store.setPartial(new range_1.Range(18, 1, 42, 1), [
                new tokensStore_1.MultilineTokens2(20, new tokensStore_1.SparseEncodedTokens(new Uint32Array([
                    0, 5, 10, 4,
                    5, 5, 10, 5,
                    10, 5, 10, 6,
                    15, 5, 10, 7,
            ...
            const lineTokens = store.addSemanticTokens(10, new lineTokens_1.LineTokens(new Uint32Array([12, 1]), `enum Enum1 {`));
        ...
        test('partial tokens 2', () => {
            ...
            store.setPartial(new range_1.Range(6, 1, 36, 2), [
                new tokensStore_1.MultilineTokens2(10, new tokensStore_1.SparseEncodedTokens(new Uint32Array([
                    0, 5, 10, 2,
                    5, 5, 10, 3,
                    10, 5, 10, 4,
                    15, 5, 10, 5,
            ...
            const lineTokens = store.addSemanticTokens(20, new lineTokens_1.LineTokens(new Uint32Array([12, 1]), `enum Enum1 {`));
            assert.strictEqual(lineTokens.getCount(), 3);
        });
        test('partial tokens 3', () => {
            ...
            const lineTokens = store.addSemanticTokens(5, new lineTokens_1.LineTokens(new Uint32Array([12, 1]), `enum Enum1 {`));
            assert.strictEqual(lineTokens.getCount(), 3);
        });
        test('issue #94133: Semantic colors stick around when using (only) range provider', () => {
            ...
            // setPartial: [1,1 -> 1,20], []
            store.setPartial(new range_1.Range(1, 1, 1, 20), []);
            const lineTokens = store.addSemanticTokens(1, new lineTokens_1.LineTokens(new Uint32Array([12, 1]), `enum Enum1 {`));
        ...
        test('bug', () => {
            function createTokens(str) {
                ...
                return new tokensStore_1.MultilineTokens2(firstLineNumber, new tokensStore_1.SparseEncodedTokens(new Uint32Array(result)));
            }
            const store = new tokensStore_1.TokensStore2();
            ...
            const lineTokens = store.addSemanticTokens(36451, new lineTokens_1.LineTokens(new Uint32Array([60, 1]), `                        if (flags & ModifierFlags.Ambient) {`));
        ...
        test('issue #95949: Identifiers are colored in bold when targetting keywords', () => {
            function createTMMetadata(foreground, fontStyle, languageId) {
                return ((languageId << 0 /* LANGUAGEID_OFFSET */)
                    | (fontStyle << 11 /* FONT_STYLE_OFFSET */)
                    | (foreground << 14 /* FOREGROUND_OFFSET */)) >>> 0;
            }
            ...
            store.set([
                new tokensStore_1.MultilineTokens2(1, new tokensStore_1.SparseEncodedTokens(new Uint32Array([
                    0, 6, 11, (1 << 14 /* FOREGROUND_OFFSET */) | 8 /* SEMANTIC_USE_FOREGROUND */,
                ])))
            ], true);
            const lineTokens = store.addSemanticTokens(1, new lineTokens_1.LineTokens(new Uint32Array([
                5, createTMMetadata(5, 2 /* Bold */, 53),
                14, createTMMetadata(1, 0 /* None */, 53),
                17, createTMMetadata(6, 0 /* None */, 53),
                18, createTMMetadata(1, 0 /* None */, 53),
            ]), `const hello = 123;`));
            const actual = toArr(lineTokens);
            assert.deepStrictEqual(actual, [
                5, createTMMetadata(5, 2 /* Bold */, 53),
                6, createTMMetadata(1, 0 /* None */, 53),
                11, createTMMetadata(1, 0 /* None */, 53),
                14, createTMMetadata(1, 0 /* None */, 53),
                17, createTMMetadata(6, 0 /* None */, 53),
                18, createTMMetadata(1, 0 /* None */, 53)
            ]);
