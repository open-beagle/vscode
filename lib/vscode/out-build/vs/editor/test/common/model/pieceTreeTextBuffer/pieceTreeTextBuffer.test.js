 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/editor/common/controller/wordCharacterClassifier", "vs/editor/common/core/position", "vs/editor/common/core/range", "vs/editor/common/model/pieceTreeTextBuffer/pieceTreeTextBufferBuilder", "vs/editor/common/model/pieceTreeTextBuffer/rbTreeBase", "vs/editor/test/common/editorTestUtils", "vs/editor/common/model/textModelSearch", "vs/base/common/strings"], function (require, exports, assert, wordCharacterClassifier_1, position_1, range_1, pieceTreeTextBufferBuilder_1, rbTreeBase_1, editorTestUtils_1, textModelSearch_1, strings_1) {
    "use strict";
    ...
    function createTextBuffer(val, normalizeEOL = true) {
        ...
        return factory.create(1 /* LF */).textBuffer.getPieceTree();
    }
    ...
    suite('snapshot', () => {
        ...
        test('immutable snapshot 1', () => {
            const model = (0, editorTestUtils_1.createTextModel)('abc\ndef');
            const snapshot = model.createSnapshot();
            model.applyEdits([
                {
    ...
    suite('chunk based search', () => {
        test('#45892. For some cases, the buffer is empty but we still try to search', () => {
            ...
            let ret = pieceTree.findMatchesLineByLine(new range_1.Range(1, 1, 1, 1), new textModelSearch_1.SearchData(/abc/, new wordCharacterClassifier_1.WordCharacterClassifier(',./'), 'abc'), true, 1000);
        ...
        test('#45770. FindInNode should not cross node boundary.', () => {
            ...
            pieceTree.insert(16, ' ');
            let ret = pieceTree.findMatchesLineByLine(new range_1.Range(1, 1, 4, 13), new textModelSearch_1.SearchData(/\[/gi, new wordCharacterClassifier_1.WordCharacterClassifier(',./'), '['), true, 1000);
        ...
        test('search searching from the middle', () => {
            ...
            let ret = pieceTree.findMatchesLineByLine(new range_1.Range(2, 3, 2, 6), new textModelSearch_1.SearchData(/a/gi, null, 'a'), true, 1000);
            assert.strictEqual(ret.length, 1);
            assert.deepStrictEqual(ret[0].range, new range_1.Range(2, 3, 2, 4));
            pieceTree.delete(4, 1);
            ret = pieceTree.findMatchesLineByLine(new range_1.Range(2, 2, 2, 5), new textModelSearch_1.SearchData(/a/gi, null, 'a'), true, 1000);
            assert.strictEqual(ret.length, 1);
