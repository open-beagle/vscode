define(["require", "exports", "assert", "vs/editor/common/config/editorOptions", "vs/editor/common/viewModel/monospaceLineBreaksComputer", "vs/editor/common/config/fontInfo", "vs/editor/common/viewModel/viewModel"], function (require, exports, assert, editorOptions_1, monospaceLineBreaksComputer_1, fontInfo_1, viewModel_1) {
    "use strict";
    ...
    function toAnnotatedText(text, lineBreakData) {
        ...
        if (lineBreakData) {
            ...
            for (let i = 0, len = text.length; i < len; i++) {
                let r = viewModel_1.LineBreakData.getOutputPositionOfInputOffset(lineBreakData.breakOffsets, i);
    ...
    function getLineBreakData(factory, tabSize, breakAfter, columnsForFullWidthChar, wrappingIndent, text, previousLineBreakData) {
        ...
        const previousLineBreakDataClone = previousLineBreakData ? new viewModel_1.LineBreakData(previousLineBreakData.breakOffsets.slice(0), previousLineBreakData.breakOffsetsVisibleColumn.slice(0), previousLineBreakData.wrappedTextIndentLength) : null;
        lineBreaksComputer.addRequest(text, previousLineBreakDataClone);
        return lineBreaksComputer.finalize()[0];
    }
    function assertLineBreaks(factory, tabSize, breakAfter, annotatedText, wrappingIndent = 0 /* None */) {
    ...
    suite('Editor ViewModel - MonospaceLineBreaksComputer', () => {
        test('MonospaceLineBreaksComputer', () => {
            let factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory('(', '\t).');
            // Empty string
        ...
        function assertIncrementalLineBreaks(factory, text, tabSize, breakAfter1, annotatedText1, breakAfter2, annotatedText2, wrappingIndent = 0 /* None */, columnsForFullWidthChar = 2) {
        ...
        test('MonospaceLineBreaksComputer - WrappingIndent.Same', () => {
            let factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory('', '\t ');
            assertLineBreaks(factory, 4, 38, ' *123456789012345678901234567890123456|7890', 1 /* Same */);
        });
        test('issue #16332: Scroll bar overlaying on top of text', () => {
            let factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory('', '\t ');
            assertLineBreaks(factory, 4, 24, 'a/ very/long/line/of/tex|t/that/expands/beyon|d/your/typical/line/|of/code/', 2 /* Indent */);
        });
        test('issue #35162: wrappingIndent not consistently working', () => {
            let factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory('', '\t ');
            let mapper = assertLineBreaks(factory, 4, 24, '                t h i s |i s |a l |o n |g l |i n |e', 2 /* Indent */);
            assert.strictEqual(mapper.wrappedTextIndentLength, '                    '.length);
        });
        test('issue #75494: surrogate pairs', () => {
            let factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory('\t', ' ');
            assertLineBreaks(factory, 4, 49, '🐇👬🌖🌞🏇🍼🐇👬🌖🌞🏇🍼🐇👬🌖🌞🏇🍼🐇👬🌖🌞🏇🍼|🐇👬🌖🌞🏇🍼🐇👬🌖🌞🏇🍼🐇👬🌖🌞🏇🍼🐇👬🌖🌞🏇🍼|🐇👬', 1 /* Same */);
        });
        test('issue #75494: surrogate pairs overrun 1', () => {
            const factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory(editorOptions_1.EditorOptions.wordWrapBreakBeforeCharacters.defaultValue, editorOptions_1.EditorOptions.wordWrapBreakAfterCharacters.defaultValue);
            assertLineBreaks(factory, 4, 4, '🐇👬|&|🌞🌖', 1 /* Same */);
        });
        test('issue #75494: surrogate pairs overrun 2', () => {
            const factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory(editorOptions_1.EditorOptions.wordWrapBreakBeforeCharacters.defaultValue, editorOptions_1.EditorOptions.wordWrapBreakAfterCharacters.defaultValue);
            assertLineBreaks(factory, 4, 17, 'factory, |"xtxtfunc|(x"🌞🏇🍼🌞🏇🍼🐇|&👬🌖🌞👬🌖🌞🏇🍼|🐇👬x"', 1 /* Same */);
        });
        test('MonospaceLineBreaksComputer - WrappingIndent.DeepIndent', () => {
            let factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory('', '\t ');
            let mapper = assertLineBreaks(factory, 4, 26, '        W e A r e T e s t |i n g D e |e p I n d |e n t a t |i o n', 3 /* DeepIndent */);
            assert.strictEqual(mapper.wrappedTextIndentLength, '                '.length);
        });
        test('issue #33366: Word wrap algorithm behaves differently around punctuation', () => {
            const factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory(editorOptions_1.EditorOptions.wordWrapBreakBeforeCharacters.defaultValue, editorOptions_1.EditorOptions.wordWrapBreakAfterCharacters.defaultValue);
            assertLineBreaks(factory, 4, 23, 'this is a line of |text, text that sits |on a line', 1 /* Same */);
        });
        test('issue #112382: Word wrap doesn\'t work well with control characters', () => {
            const factory = new monospaceLineBreaksComputer_1.MonospaceLineBreaksComputerFactory(editorOptions_1.EditorOptions.wordWrapBreakBeforeCharacters.defaultValue, editorOptions_1.EditorOptions.wordWrapBreakAfterCharacters.defaultValue);
            assertLineBreaks(factory, 4, 6, '\x06\x06\x06|\x06\x06\x06', 1 /* Same */);
        });
    });
});
