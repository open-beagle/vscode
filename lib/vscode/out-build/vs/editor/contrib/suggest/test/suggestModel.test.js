define(["require", "exports", "assert", "vs/base/common/lifecycle", "vs/base/common/uri", "vs/editor/browser/controller/coreCommands", "vs/editor/common/core/editOperation", "vs/editor/common/core/range", "vs/editor/common/core/selection", "vs/editor/common/core/token", "vs/editor/common/modes", "vs/editor/common/modes/languageConfigurationRegistry", "vs/editor/common/modes/nullMode", "vs/editor/contrib/snippet/snippetController2", "vs/editor/contrib/suggest/suggestController", "vs/editor/contrib/suggest/suggestModel", "vs/editor/test/browser/testCodeEditor", "vs/editor/test/common/mocks/mockMode", "vs/platform/instantiation/common/serviceCollection", "vs/platform/storage/common/storage", "vs/platform/telemetry/common/telemetry", "vs/platform/telemetry/common/telemetryUtils", "vs/editor/contrib/suggest/suggestMemory", "vs/platform/keybinding/common/keybinding", "vs/platform/keybinding/test/common/mockKeybindingService", "vs/editor/test/common/editorTestUtils", "vs/base/test/common/mock", "vs/platform/log/common/log"], function (require, exports, assert, lifecycle_1, uri_1, coreCommands_1, editOperation_1, range_1, selection_1, token_1, modes_1, languageConfigurationRegistry_1, nullMode_1, snippetController2_1, suggestController_1, suggestModel_1, testCodeEditor_1, mockMode_1, serviceCollection_1, storage_1, telemetry_1, telemetryUtils_1, suggestMemory_1, keybinding_1, mockKeybindingService_1, editorTestUtils_1, mock_1, log_1) {
    ...
    suite('SuggestModel - Context', function () {
        ...
        test('Context - shouldAutoTrigger', function () {
            const model = (0, editorTestUtils_1.createTextModel)('Das Pferd frisst keinen Gurkensalat - Philipp Reis 1861.\nWer hat\'s erfunden?');
        ...
        test('shouldAutoTrigger at embedded language boundaries', () => {
            ...
            const model = (0, editorTestUtils_1.createTextModel)('a<xx>a<x>', undefined, outerMode.getLanguageIdentifier());
            disposables.push(model);
    ...
    suite('SuggestModel - TriggerAndCancelOracle', function () {
        ...
        setup(function () {
            ...
            model = (0, editorTestUtils_1.createTextModel)('abc def', undefined, undefined, uri_1.URI.parse('test:somefile.ttt'));
            disposables.push(model);
        });
        function withOracle(callback) {
            return new Promise((resolve, reject) => {
                const editor = createMockEditor(model);
                const oracle = new suggestModel_1.SuggestModel(editor, new class extends (0, mock_1.mock)() {
                    computeWordRanges() {
                ...
                }, telemetryUtils_1.NullTelemetryService, new log_1.NullLogService());
        ...
        test('trigger - on type', function () {
            ...
            return withOracle((model, editor) => {
                return assertEvent(model.onDidSuggest, () => {
                    ...
                    editor.trigger('keyboard', "type" /* Type */, { text: 'd' });
                }, event => {
                    assert.strictEqual(event.auto, true);
            ...
            return withOracle((model, editor) => {
                return assertEvent(model.onDidSuggest, () => {
                    ...
                    return assertEvent(model.onDidSuggest, () => {
                        ...
                        editor.trigger('keyboard', "type" /* Type */, { text: 'My' });
                    }, event => {
                        assert.strictEqual(event.auto, true);
                        assert.strictEqual(event.completionModel.items.length, 1);
                        const [first] = event.completionModel.items;
                        ...
                        return assertEvent(model.onDidSuggest, () => {
                            ...
                            editor.trigger('keyboard', "type" /* Type */, { text: ' ' });
                        }, event => {
                            assert.strictEqual(event.auto, true);
                            assert.strictEqual(event.completionModel.items.length, 1);
                            const [first] = event.completionModel.items;
        ...
        test('#21484: Trigger character always force a new completion session', function () {
            ...
            return withOracle((model, editor) => {
                return assertEvent(model.onDidSuggest, () => {
                    editor.setPosition({ lineNumber: 1, column: 1 });
                    editor.trigger('keyboard', "type" /* Type */, { text: 'foo' });
                }, event => {
                    assert.strictEqual(event.auto, true);
                    assert.strictEqual(event.completionModel.items.length, 1);
                    const [first] = event.completionModel.items;
        ...
        test('Trigger character is provided in suggest context', function () {
            ...
            disposables.push(modes_1.CompletionProviderRegistry.register({ scheme: 'test' }, {
                ...
                provideCompletionItems(doc, pos, context) {
                    assert.strictEqual(context.triggerKind, 1 /* TriggerCharacter */);
        ...
        test('Completion unexpectedly triggers on second keypress of an edit group in a snippet #43523', function () {
            ...
            return withOracle((model, editor) => {
                return assertEvent(model.onDidSuggest, () => {
                    ...
                    editor.trigger('keyboard', "type" /* Type */, { text: 'e' });
                }, event => {
                    assert.strictEqual(event.auto, true);
                    assert.strictEqual(event.completionModel.items.length, 1);
                    const [first] = event.completionModel.items;
        ...
        test('Fails to render completion details #47988', function () {
            ...
            return withOracle(async (model, editor) => {
                ...
                await assertEvent(model.onDidSuggest, () => {
                    editor.trigger('keyboard', "type" /* Type */, { text: 'o' });
                }, event => {
                    assert.strictEqual(event.auto, true);
                    assert.strictEqual(event.completionModel.items.length, 2);
                    // clean up
        ...
        test('Trigger (full) completions when (incomplete) completions are already active #99504', function () {
            ...
            return withOracle(async (model, editor) => {
                await assertEvent(model.onDidSuggest, () => {
                    editor.setValue('');
                    editor.setSelection(new selection_1.Selection(1, 1, 1, 1));
                    editor.trigger('keyboard', "type" /* Type */, { text: 'Z' });
                }, event => {
                    assert.strictEqual(event.auto, true);
                    assert.strictEqual(event.completionModel.items.length, 1);
