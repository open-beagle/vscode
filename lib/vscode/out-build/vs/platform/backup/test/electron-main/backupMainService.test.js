 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/base/common/platform", "fs", "os", "vs/base/common/path", "vs/base/node/pfs", "vs/base/common/uri", "vs/platform/environment/electron-main/environmentMainService", "vs/platform/environment/node/argv", "vs/platform/backup/electron-main/backupMainService", "vs/platform/files/common/files", "vs/platform/configuration/test/common/testConfigurationService", "vs/platform/log/common/log", "crypto", "vs/base/test/node/testUtils", "vs/base/common/network", "vs/base/common/resources", "vs/platform/product/common/product"], function (require, exports, assert, platform, fs, os, path, pfs, uri_1, environmentMainService_1, argv_1, backupMainService_1, files_1, testConfigurationService_1, log_1, crypto_1, testUtils_1, network_1, resources_1, product_1) {
    "use strict";
    ...
    (0, testUtils_1.flakySuite)('BackupMainService', () => {
        ...
        async function createBackupFolder(backupFolder) {
            if (!fs.existsSync(backupFolder)) {
                ...
                await pfs.writeFile(path.join(backupFolder, network_1.Schemas.file, 'foo.txt'), 'Hello');
            }
        }
        ...
        test('should always store the workspace path in workspaces.json using the case given, regardless of whether the file system is case-sensitive (folder workspace)', async () => {
            ...
            const buffer = await fs.promises.readFile(backupWorkspacesPath, 'utf-8');
            const json = JSON.parse(buffer);
            assert.deepStrictEqual(json.folderURIWorkspaces, [uri_1.URI.file(fooFile.fsPath.toUpperCase()).toString()]);
        });
        ...
        suite('getWorkspaceHash', () => {
            (platform.isLinux ? test.skip : test)('should ignore case on Windows and Mac', () => {
                if (platform.isMacintosh) {
                    assert.strictEqual(service.getFolderHash(uri_1.URI.file('/foo')), service.getFolderHash(uri_1.URI.file('/FOO')));
                }
                if (platform.isWindows) {
                    assert.strictEqual(service.getFolderHash(uri_1.URI.file('c:\\foo')), service.getFolderHash(uri_1.URI.file('C:\\FOO')));
                }
            });
        });
        ...
        suite('getDirtyWorkspaces', () => {
            test('should report if a workspace or folder has backups', async () => {
                ...
                try {
                    await fs.promises.mkdir(path.join(folderBackupPath, network_1.Schemas.file), { recursive: true });
                    await fs.promises.mkdir(path.join(workspaceBackupPath, network_1.Schemas.untitled), { recursive: true });
                }
                catch (error) {
                ...
                for (const dirtyWorkpspace of dirtyWorkspaces) {
                    if (uri_1.URI.isUri(dirtyWorkpspace)) {
                        if ((0, resources_1.isEqual)(fooFile, dirtyWorkpspace)) {
                            found++;
                        }
                    }
                    else {
                        if ((0, resources_1.isEqual)(backupWorkspaceInfo.workspace.configPath, dirtyWorkpspace.configPath)) {
                            found++;
                        }
                    }
                }
