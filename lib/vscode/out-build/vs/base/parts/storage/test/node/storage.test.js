 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "vs/base/parts/storage/node/storage", "vs/base/parts/storage/common/storage", "vs/base/common/path", "os", "fs", "assert", "vs/base/node/pfs", "vs/base/common/async", "vs/base/common/event", "vs/base/common/platform", "vs/base/test/node/testUtils", "vs/base/common/uuid"], function (require, exports, storage_1, storage_2, path_1, os_1, fs_1, assert_1, pfs_1, async_1, event_1, platform_1, testUtils_1, uuid_1) {
    "use strict";
    ...
    (0, testUtils_1.flakySuite)('Storage Library', function () {
        ...
        test('basics', async () => {
            const storage = new storage_2.Storage(new storage_1.SQLiteStorageDatabase((0, path_1.join)(testDir, 'storage.db')));
            await storage.init();
            // Empty fallbacks
        ...
        test('close flushes data', async () => {
            let storage = new storage_2.Storage(new storage_1.SQLiteStorageDatabase((0, path_1.join)(testDir, 'storage.db')));
            await storage.init();
            ...
            (0, assert_1.strictEqual)(setPromiseResolved, true);
            (0, assert_1.strictEqual)(flushPromiseResolved, true);
            storage = new storage_2.Storage(new storage_1.SQLiteStorageDatabase((0, path_1.join)(testDir, 'storage.db')));
            await storage.init();
            (0, assert_1.strictEqual)(storage.get('foo'), 'bar');
            (0, assert_1.strictEqual)(storage.get('bar'), 'foo');
            await storage.close();
        ...
        test('corrupt DB recovers', async () => {
            ...
            await (0, pfs_1.writeFile)(storageFile, 'This is a broken DB');
            await storage.set('foo', 'bar');
            (0, assert_1.strictEqual)(storage.get('bar'), 'foo');
            (0, assert_1.strictEqual)(storage.get('foo'), 'bar');
            await storage.close();
    ...
    (0, testUtils_1.flakySuite)('SQLite Storage Library', function () {
        ...
        test('basics (corrupt DB falls back to empty DB)', async () => {
            ...
            await (0, pfs_1.writeFile)(corruptDBPath, 'This is a broken DB');
            let expectedError;
        ...
        test('basics (corrupt DB restores from previous backup)', async () => {
            ...
            await storage.updateItems({ insert: items });
            await storage.close();
            await (0, pfs_1.writeFile)(storagePath, 'This is now a broken DB');
        ...
        test('basics (corrupt DB falls back to empty DB if backup is corrupt)', async () => {
            ...
            await storage.close();
            await (0, pfs_1.writeFile)(storagePath, 'This is now a broken DB');
            await (0, pfs_1.writeFile)(`${storagePath}.backup`, 'This is now also a broken DB');
            storage = new storage_1.SQLiteStorageDatabase(storagePath);
            const storedItems = await storage.getItems();
            (0, assert_1.strictEqual)(storedItems.size, 0);
        ...
        (platform_1.isWindows ? test.skip /* Windows will fail to write to open DB due to locking */ : test)('basics (DB that becomes corrupt during runtime stores all state from cache on close)', async () => {
            ...
            (0, assert_1.strictEqual)(await (0, pfs_1.exists)(backupPath), true);
            storage = new storage_1.SQLiteStorageDatabase(storagePath);
            ...
            await fs_1.promises.unlink(backupPath); // also test that the recovery DB is backed up properly
            let recoveryCalled = false;
            await storage.close(() => {
                recoveryCalled = true;
                return items;
        ...
        test('real world example', async function () {
            let storage = new storage_1.SQLiteStorageDatabase((0, path_1.join)(testdir, 'storage.db'));
            ...
            (0, assert_1.strictEqual)(storedItems.size, items1.size + items2.size + items3.size);
            await storage.close();
            storage = new storage_1.SQLiteStorageDatabase((0, path_1.join)(testdir, 'storage.db'));
            storedItems = await storage.getItems();
            (0, assert_1.strictEqual)(storedItems.size, items1.size + items2.size + items3.size);
            await storage.close();
        });
        ...
        test('multiple concurrent writes execute in sequence', async () => {
            ...
            const storage = new TestStorage(new storage_1.SQLiteStorageDatabase((0, path_1.join)(testdir, 'storage.db')));
            await storage.init();
            storage.set('foo', 'bar');
        ...
        test('lots of INSERT & DELETE (below inline max)', async () => {
            const storage = new storage_1.SQLiteStorageDatabase((0, path_1.join)(testdir, 'storage.db'));
            const items = new Map();
            const keys = new Set();
            for (let i = 0; i < 200; i++) {
