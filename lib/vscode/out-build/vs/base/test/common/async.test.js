 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/base/common/async", "vs/base/common/errors", "vs/base/common/uri", "vs/base/common/cancellation", "vs/base/common/event"], function (require, exports, assert, async, errors_1, uri_1, cancellation_1, event_1) {
    "use strict";
    ...
    suite('Async', () => {
        test('cancelablePromise - set token, don\'t wait for inner promise', function () {
            ...
            let result = promise.then(_ => assert.ok(false), err => {
                ...
                assert.ok((0, errors_1.isPromiseCanceledError)(err));
            });
            promise.cancel();
        ...
        test('Queue - events', function () {
            let queue = new async.Queue();
            let finished = false;
            const onFinished = event_1.Event.toPromise(queue.onFinished);
            let res = [];
        ...
        suite('Promises.allSettled', () => {
            test('resolves', async () => {
                ...
                const result = await async.Promises.allSettled([p1, p2, p3]);
            ...
            test('can empty', async () => {
                const result = await async.Promises.allSettled([]);
