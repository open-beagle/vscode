...
define(["require", "exports", "assert", "vs/base/common/event", "vs/base/browser/ui/splitview/splitview"], function (require, exports, assert, event_1, splitview_1) {
    ...
    suite('Splitview', () => {
        ...
        test('sashes are properly enabled/disabled', () => {
            ...
            assert.strictEqual(sashes[0].state, 3 /* Enabled */, 'first sash is enabled');
            assert.strictEqual(sashes[1].state, 3 /* Enabled */, 'second sash is enabled');
            splitview.layout(60);
            assert.strictEqual(sashes[0].state, 0 /* Disabled */, 'first sash is disabled');
            assert.strictEqual(sashes[1].state, 0 /* Disabled */, 'second sash is disabled');
            splitview.layout(20);
            assert.strictEqual(sashes[0].state, 0 /* Disabled */, 'first sash is disabled');
            assert.strictEqual(sashes[1].state, 0 /* Disabled */, 'second sash is disabled');
            splitview.layout(200);
            ...
            assert.strictEqual(sashes[0].state, 1 /* Minimum */, 'first sash is enabled');
            assert.strictEqual(sashes[1].state, 1 /* Minimum */, 'second sash is enabled');
            view2.maximumSize = 200;
            assert.strictEqual(sashes[0].state, 1 /* Minimum */, 'first sash is enabled');
            assert.strictEqual(sashes[1].state, 1 /* Minimum */, 'second sash is enabled');
