...
define(["require", "exports", "vs/platform/registry/common/platform", "vs/platform/workspaces/common/workspaces", "vs/base/common/path", "vs/base/common/resources", "vs/platform/files/common/files", "vs/base/common/uri", "vs/workbench/services/textfile/common/textfiles", "vs/base/common/buffer", "vs/base/common/network", "vs/base/browser/dnd", "vs/base/common/labels", "vs/base/common/mime", "vs/base/common/platform", "vs/editor/browser/editorBrowser", "vs/workbench/common/editor", "vs/workbench/services/editor/common/editorService", "vs/base/common/lifecycle", "vs/base/browser/dom", "vs/workbench/services/workspaces/common/workspaceEditing", "vs/base/common/types", "vs/workbench/services/host/browser/host", "vs/workbench/services/workingCopy/common/workingCopyBackup", "vs/base/common/event", "vs/workbench/services/workingCopy/common/workingCopy"], function (require, exports, platform_1, workspaces_1, path_1, resources_1, files_1, uri_1, textfiles_1, buffer_1, network_1, dnd_1, labels_1, mime_1, platform_2, editorBrowser_1, editor_1, editorService_1, lifecycle_1, dom_1, workspaceEditing_1, types_1, host_1, workingCopyBackup_1, event_1, workingCopy_1) {
    "use strict";
        ...
        if (e.dataTransfer && e.dataTransfer.types.length > 0) {
            ...
            if (e.dataTransfer && e.dataTransfer.files) {
                for (let i = 0; i < e.dataTransfer.files.length; i++) {
                    ...
                    if ((file === null || file === void 0 ? void 0 : file.path /* Electron only */) && !resources.some(resource => resource.resource.fsPath === file.path) /* prevent duplicates */) {
                        try {
    ...
    let ResourcesDropHandler = class ResourcesDropHandler {
        constructor(options, fileService, workspacesService, textFileService, workingCopyBackupService, editorService, workspaceEditingService, hostService) {
        ...
        async handleDrop(event, resolveTargetGroup, afterDrop, targetIndex) {
            const untitledOrFileResources = extractResources(event).filter(resource => this.fileService.canHandleResource(resource.resource) || resource.resource.scheme === network_1.Schemas.untitled);
            ...
            const recentFiles = untitledOrFileResources.filter(untitledOrFileResource => untitledOrFileResource.isExternal && untitledOrFileResource.resource.scheme === network_1.Schemas.file).map(d => ({ fileUri: d.resource }));
        ...
        async doHandleDrop(untitledOrFileResources) {
            ...
            const dirtyEditors = untitledOrFileResources.filter(untitledOrFileResource => !untitledOrFileResource.isExternal && typeof untitledOrFileResource.dirtyContent === 'string');
            ...
            if (this.options.allowWorkspaceOpen) {
                const externalFileOnDiskResources = untitledOrFileResources.filter(untitledOrFileResource => untitledOrFileResource.isExternal && untitledOrFileResource.resource.scheme === network_1.Schemas.file).map(d => d.resource);
        ...
        async handleDirtyEditorDrop(droppedDirtyEditor) {
            const fileEditorFactory = platform_1.Registry.as(editor_1.EditorExtensions.EditorInputFactories).getFileEditorInputFactory();
            ...
            if (typeof droppedDirtyEditor.dirtyContent === 'string') {
                try {
                    await this.workingCopyBackupService.backup({ resource: droppedDirtyEditor.resource, typeId: workingCopy_1.NO_TYPE_ID }, (0, buffer_1.bufferToReadable)(buffer_1.VSBuffer.fromString(droppedDirtyEditor.dirtyContent)));
                }
                catch (e) {
                    // Ignore error
                }
        ...
        async handleWorkspaceFileDrop(fileOnDiskResources) {
            ...
            await Promise.all(fileOnDiskResources.map(async (fileOnDiskResource) => {
                // Check for Workspace
                if ((0, workspaces_1.hasWorkspaceFileExtension)(fileOnDiskResource)) {
    ...
    function fillResourceDataTransfers(accessor, resources, optionsCallback, event) {
        ...
        const lineDelimiter = platform_2.isWindows ? '\r\n' : '\n';
        event.dataTransfer.setData(dnd_1.DataTransfers.TEXT, sources.map(source => source.resource.scheme === network_1.Schemas.file ? (0, path_1.normalize)((0, labels_1.normalizeDriveLetter)(source.resource.fsPath)) : source.resource.toString()).join(lineDelimiter));
        // Download URL: enables support to drag a tab as file to desktop (only single file supported)
        if (!sources[0].isDirectory) {
            event.dataTransfer.setData(dnd_1.DataTransfers.DOWNLOAD_URL, [mime_1.MIME_BINARY, (0, resources_1.basename)(sources[0].resource), network_1.FileAccess.asBrowserUri(sources[0].resource).toString()].join(':'));
        }
        ...
        if (files.length) {
            event.dataTransfer.setData(dnd_1.DataTransfers.RESOURCES, JSON.stringify(files.map(file => file.resource.toString())));
        }
                ...
                options = {
                    viewState: (() => {
                        ...
                        for (const textEditorControl of textEditorControls) {
                            if ((0, editorBrowser_1.isCodeEditor)(textEditorControl)) {
                                const model = textEditorControl.getModel();
                                if ((0, resources_1.isEqual)(model === null || model === void 0 ? void 0 : model.uri, file.resource)) {
                                    return (0, types_1.withNullAsUndefined)(textEditorControl.saveViewState());
                                }
                            }
                        }
                        return undefined;
    ...
    class DragAndDropObserver extends lifecycle_1.Disposable {
        ...
        registerListeners() {
            this._register((0, dom_1.addDisposableListener)(this.element, dom_1.EventType.DRAG_ENTER, (e) => {
                this.counter++;
                this.callbacks.onDragEnter(e);
            }));
            this._register((0, dom_1.addDisposableListener)(this.element, dom_1.EventType.DRAG_OVER, (e) => {
                e.preventDefault(); // needed so that the drop event fires (https://stackoverflow.com/questions/21339924/drop-event-not-firing-in-chrome)
                if (this.callbacks.onDragOver) {
                    this.callbacks.onDragOver(e);
                }
            }));
            this._register((0, dom_1.addDisposableListener)(this.element, dom_1.EventType.DRAG_LEAVE, (e) => {
                this.counter--;
                if (this.counter === 0) {
                    this.callbacks.onDragLeave(e);
                }
            }));
            this._register((0, dom_1.addDisposableListener)(this.element, dom_1.EventType.DRAG_END, (e) => {
                this.counter = 0;
                this.callbacks.onDragEnd(e);
            }));
            this._register((0, dom_1.addDisposableListener)(this.element, dom_1.EventType.DROP, (e) => {
                this.counter = 0;
