...
define(["require", "exports", "assert", "vs/workbench/common/memento", "vs/workbench/test/common/workbenchTestServices"], function (require, exports, assert, memento_1, workbenchTestServices_1) {
    ...
    suite('Memento', () => {
        ...
        test('Loading and Saving Memento with Scopes', () => {
            ...
            // Global
            let memento = myMemento.getMemento(0 /* GLOBAL */, 1 /* MACHINE */);
            memento.foo = [1, 2, 3];
            let globalMemento = myMemento.getMemento(0 /* GLOBAL */, 1 /* MACHINE */);
            assert.deepStrictEqual(globalMemento, memento);
            // Workspace
            memento = myMemento.getMemento(1 /* WORKSPACE */, 1 /* MACHINE */);
            assert(memento);
            memento.foo = 'Hello World';
            myMemento.saveMemento();
            // Global
            memento = myMemento.getMemento(0 /* GLOBAL */, 1 /* MACHINE */);
            assert.deepStrictEqual(memento, { foo: [1, 2, 3] });
            globalMemento = myMemento.getMemento(0 /* GLOBAL */, 1 /* MACHINE */);
            assert.deepStrictEqual(globalMemento, memento);
            // Workspace
            memento = myMemento.getMemento(1 /* WORKSPACE */, 1 /* MACHINE */);
            assert.deepStrictEqual(memento, { foo: 'Hello World' });
            // Assert the Mementos are stored properly in storage
            assert.deepStrictEqual(JSON.parse(storage.get('memento/memento.test', 0 /* GLOBAL */)), { foo: [1, 2, 3] });
            assert.deepStrictEqual(JSON.parse(storage.get('memento/memento.test', 1 /* WORKSPACE */)), { foo: 'Hello World' });
            // Delete Global
            memento = myMemento.getMemento(context, 1 /* MACHINE */);
            delete memento.foo;
            // Delete Workspace
            memento = myMemento.getMemento(1 /* WORKSPACE */, 1 /* MACHINE */);
            delete memento.foo;
        ...
        test('Save and Load', () => {
            let myMemento = new memento_1.Memento('memento.test', storage);
            // Global
            let memento = myMemento.getMemento(context, 1 /* MACHINE */);
            memento.foo = [1, 2, 3];
            // Workspace
            memento = myMemento.getMemento(1 /* WORKSPACE */, 1 /* MACHINE */);
            assert(memento);
        ...
        test('Save and Load - 2 Components with same id', () => {
            ...
            let memento = myMemento.getMemento(context, 1 /* MACHINE */);
            memento.foo = [1, 2, 3];
            memento = myMemento2.getMemento(context, 1 /* MACHINE */);
            memento.bar = [1, 2, 3];
            // Workspace
            memento = myMemento.getMemento(1 /* WORKSPACE */, 1 /* MACHINE */);
            assert(memento);
            memento.foo = 'Hello World';
            memento = myMemento2.getMemento(1 /* WORKSPACE */, 1 /* MACHINE */);
            assert(memento);
            ...
            memento = myMemento2.getMemento(context, 1 /* MACHINE */);
            assert.deepStrictEqual(memento, { foo: [1, 2, 3], bar: [1, 2, 3] });
            globalMemento = myMemento2.getMemento(0 /* GLOBAL */, 1 /* MACHINE */);
            assert.deepStrictEqual(globalMemento, memento);
            // Workspace
            memento = myMemento.getMemento(1 /* WORKSPACE */, 1 /* MACHINE */);
