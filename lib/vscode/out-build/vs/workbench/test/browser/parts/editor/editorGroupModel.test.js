    ...
    suite('Workbench editor group model', () => {
        ...
        function closeEditors(group, except, direction) {
            ...
            // Close to the right
                ...
                for (let i = group.getEditors(1 /* SEQUENTIAL */).length - 1; i > index; i--) {
                    group.closeEditor(group.getEditorByIndex(i));
                }
            }
            // Both directions
            else {
                group.getEditors(0 /* MOST_RECENTLY_ACTIVE */).filter(editor => !editor.matches(except)).forEach(editor => group.closeEditor(editor));
            }
        }
        ...
        setup(() => {
            TestEditorInputSerializer.disableSerialize = false;
            TestEditorInputSerializer.disableDeserialize = false;
            disposables.add(platform_1.Registry.as(editor_1.EditorExtensions.EditorInputFactories).registerEditorInputSerializer('testEditorInputForGroups', TestEditorInputSerializer));
        });
        teardown(() => {
            disposables.clear();
            index = 1;
        ...
        test('group serialization', function () {
            inst().invokeFunction(accessor => platform_1.Registry.as(editor_1.EditorExtensions.EditorInputFactories).start(accessor));
            const group = createEditorGroupModel();
            const input1 = input();
            const input2 = input();
            const input3 = input();
            ...
            assert.strictEqual(deserialized.getEditors(1 /* SEQUENTIAL */).length, 3);
            assert.strictEqual(deserialized.getEditors(0 /* MOST_RECENTLY_ACTIVE */).length, 3);
            ...
            assert.strictEqual(deserialized.getEditors(1 /* SEQUENTIAL */).length, 0);
            assert.strictEqual(deserialized.getEditors(0 /* MOST_RECENTLY_ACTIVE */).length, 0);
        ...
        test('One Editor', function () {
            ...
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */).length, 1);
        ...
        test('Multiple Editors - Pinned and Active', function () {
            ...
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */).length, 3);
            assert.strictEqual(group.activeEditor, input3);
        ...
        test('Multiple Editors - Preview editor moves to the side of the active one', function () {
            ...
            group.openEditor(input2, { pinned: true, active: true });
            group.openEditor(input3, { pinned: true, active: true });
            assert.strictEqual(input3, group.getEditors(1 /* SEQUENTIAL */)[2]);
            const input4 = input();
            group.openEditor(input4, { pinned: false, active: true }); // this should cause the preview editor to move after input3
            assert.strictEqual(input4, group.getEditors(1 /* SEQUENTIAL */)[2]);
        });
        test('Multiple Editors - Pinned and Active (DEFAULT_OPEN_EDITOR_DIRECTION = Direction.LEFT)', function () {
            ...
            group.openEditor(input2, { pinned: true, active: true });
            group.openEditor(input3, { pinned: true, active: true });
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input3);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], input2);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[2], input1);
            closeAllEditors(group);
            assert.strictEqual(events.closed.length, 3);
            assert.strictEqual(group.count, 0);
        });
        ...
        test('Multiple Editors - set active', function () {
            ...
            group.openEditor(input3, { pinned: false, active: true });
            assert.strictEqual(group.activeEditor, input3);
            let mru = group.getEditors(0 /* MOST_RECENTLY_ACTIVE */);
            assert.strictEqual(mru[0], input3);
            assert.strictEqual(mru[1], input2);
            assert.strictEqual(mru[2], input1);
            ...
            assert.strictEqual(group.isActive(input2), false);
            assert.strictEqual(group.isActive(input3), false);
            mru = group.getEditors(0 /* MOST_RECENTLY_ACTIVE */);
            assert.strictEqual(mru[0], input1);
            assert.strictEqual(mru[1], input3);
            assert.strictEqual(mru[2], input2);
        });
        test('Multiple Editors - pin and unpin', function () {
            ...
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input2);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], input3);
            assert.strictEqual(events.closed[0].editor, input1);
        ...
        test('Multiple Editors - closing picks next from MRU list', function () {
            ...
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[0], input5);
        ...
        test('Multiple Editors - move editor', function () {
            ...
            assert.strictEqual(events.moved[0], input1);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input2);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], input1);
            group.setActive(input1);
            group.openEditor(input3, { pinned: true, active: true });
            group.openEditor(input4, { pinned: true, active: true });
            group.openEditor(input5, { pinned: true, active: true });
            ...
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input4);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], input2);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[2], input1);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[3], input3);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[4], input5);
            group.moveEditor(input4, 3);
            group.moveEditor(input2, 1);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input1);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], input2);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[2], input3);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[3], input4);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[4], input5);
            ...
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input5);
            group.moveEditor(input1, 100);
            assert.strictEqual(events.moved.length, 6);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[4], input1);
        });
        test('Multiple Editors - move editor across groups', function () {
            ...
            assert.strictEqual(group1.getEditors(1 /* SEQUENTIAL */)[0], g1_input1);
            assert.strictEqual(group1.getEditors(1 /* SEQUENTIAL */)[1], g2_input1);
            assert.strictEqual(group1.getEditors(1 /* SEQUENTIAL */)[2], g1_input2);
        });
        test('Multiple Editors - move editor across groups (input already exists in group 1)', function () {
            ...
            assert.strictEqual(group1.count, 3);
            assert.strictEqual(group1.getEditors(1 /* SEQUENTIAL */)[0], g1_input2);
            assert.strictEqual(group1.getEditors(1 /* SEQUENTIAL */)[1], g1_input1);
            assert.strictEqual(group1.getEditors(1 /* SEQUENTIAL */)[2], g1_input3);
        });
        test('Multiple Editors - Pinned & Non Active', function () {
            ...
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input1);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], input3);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[2], input2);
            assert.strictEqual(group.isPinned(input1), false);
            assert.strictEqual(group.isPinned(input2), true);
            assert.strictEqual(group.isPinned(input3), true);
            assert.strictEqual(group.count, 3);
        ...
        test('Multiple Editors - Close Others, Close Left, Close Right', function () {
            ...
            // Close Left
            assert.strictEqual(group.activeEditor, input3);
            closeEditors(group, group.activeEditor, 0 /* LEFT */);
            assert.strictEqual(group.activeEditor, input3);
            assert.strictEqual(group.count, 3);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input3);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], input4);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[2], input5);
            closeAllEditors(group);
            group.openEditor(input1, { active: true, pinned: true });
            group.openEditor(input2, { active: true, pinned: true });
            group.openEditor(input3, { active: true, pinned: true });
            ...
            // Close Right
            assert.strictEqual(group.activeEditor, input3);
            closeEditors(group, group.activeEditor, 1 /* RIGHT */);
            assert.strictEqual(group.activeEditor, input3);
            assert.strictEqual(group.count, 3);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], input1);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], input2);
        ...
        test('Multiple Editors - real user example', function () {
            ...
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], indexHtml);
            assert.strictEqual(group.count, 1);
            ...
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], styleCss);
            assert.strictEqual(group.count, 1);
            ...
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], styleCss);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], testJs);
            assert.strictEqual(group.count, 2);
            ...
            assert.strictEqual(group.isPinned(testJs), true);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[0], testJs);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], indexHtml2);
            assert.strictEqual(group.count, 2);
            // make test.js active
            ...
            assert.ok(group.getEditors(1 /* SEQUENTIAL */)[0].matches(testJs));
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */)[1], otherTs);
            assert.ok(group.getEditors(1 /* SEQUENTIAL */)[2].matches(indexHtml));
        ...
        test('Multiple Groups, Multiple editors - persist', function () {
            ...
            assert.strictEqual(group1.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[0].matches(g1_input2), true);
            assert.strictEqual(group1.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[1].matches(g1_input3), true);
            assert.strictEqual(group1.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[2].matches(g1_input1), true);
            assert.strictEqual(group2.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[0].matches(g2_input1), true);
            assert.strictEqual(group2.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[1].matches(g2_input3), true);
            assert.strictEqual(group2.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[2].matches(g2_input2), true);
            // Create model again - should load from storage
        ...
        test('Single group, multiple editors - persist (some not persistable)', function () {
            ...
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[0].matches(nonSerializableInput2), true);
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[1].matches(serializableInput2), true);
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[2].matches(serializableInput1), true);
            // Create model again - should load from storage
            group = inst.createInstance(editorGroupModel_1.EditorGroupModel, group.serialize());
            assert.strictEqual(group.count, 2);
            assert.strictEqual(group.activeEditor.matches(serializableInput2), true);
            assert.strictEqual(group.previewEditor, null);
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[0].matches(serializableInput2), true);
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */)[1].matches(serializableInput1), true);
        });
        ...
        test('Multiple groups, multiple editors - persist (some not persistable, causes empty group)', function () {
            ...
            assert.strictEqual(group1.getEditors(1 /* SEQUENTIAL */)[0].matches(serializableInput1), true);
            assert.strictEqual(group1.getEditors(1 /* SEQUENTIAL */)[1].matches(serializableInput2), true);
        });
        ...
        test('Sticky Editors', function () {
            ...
            group.openEditor(input3, { pinned: false, active: true });
            assert.strictEqual(group.stickyCount, 0);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */).length, 3);
            assert.strictEqual(group.getEditors(1 /* SEQUENTIAL */, { excludeSticky: true }).length, 3);
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */).length, 3);
            assert.strictEqual(group.getEditors(0 /* MOST_RECENTLY_ACTIVE */, { excludeSticky: true }).length, 3);
            ...
            let sequentialAllEditors = group.getEditors(1 /* SEQUENTIAL */);
            assert.strictEqual(sequentialAllEditors.length, 3);
            let sequentialEditorsExcludingSticky = group.getEditors(1 /* SEQUENTIAL */, { excludeSticky: true });
            ...
            let mruAllEditors = group.getEditors(0 /* MOST_RECENTLY_ACTIVE */);
            assert.strictEqual(mruAllEditors.length, 3);
            let mruEditorsExcludingSticky = group.getEditors(0 /* MOST_RECENTLY_ACTIVE */, { excludeSticky: true });
            ...
            assert.strictEqual(group.indexOf(input3), 0);
            sequentialAllEditors = group.getEditors(1 /* SEQUENTIAL */);
            assert.strictEqual(sequentialAllEditors.length, 3);
            sequentialEditorsExcludingSticky = group.getEditors(1 /* SEQUENTIAL */, { excludeSticky: true });
            assert.strictEqual(sequentialEditorsExcludingSticky.length, 1);
            assert.ok(sequentialEditorsExcludingSticky.indexOf(input1) >= 0);
            mruAllEditors = group.getEditors(0 /* MOST_RECENTLY_ACTIVE */);
            assert.strictEqual(mruAllEditors.length, 3);
            mruEditorsExcludingSticky = group.getEditors(0 /* MOST_RECENTLY_ACTIVE */, { excludeSticky: true });
