...
define(["require", "exports", "assert", "vs/base/common/keyCodes", "vs/base/common/keybindingLabels", "vs/base/common/scanCode", "vs/platform/keybinding/common/usLayoutResolvedKeybinding", "vs/workbench/services/keybinding/common/macLinuxKeyboardMapper", "vs/workbench/services/keybinding/test/electron-browser/keyboardMapperTestUtils"], function (require, exports, assert, keyCodes_1, keybindingLabels_1, scanCode_1, usLayoutResolvedKeybinding_1, macLinuxKeyboardMapper_1, keyboardMapperTestUtils_1) {
    ...
    suite('keyboardMapper - MAC de_ch', () => {
        let mapper;
        suiteSetup(async () => {
            const _mapper = await createKeyboardMapper(false, 'mac_de_ch', 2 /* Macintosh */);
            mapper = _mapper;
        });
        test('mapping', () => {
        ...
        test('resolveKeybinding Cmd+K Cmd+\\', () => {
            _assertResolveKeybinding((0, keyCodes_1.KeyChord)(2048 /* CtrlCmd */ | 41 /* KEY_K */, 2048 /* CtrlCmd */ | 88 /* US_BACKSLASH */), [{
                    label: '⌘K ⌃⇧⌥⌘7',
    ...
    suite('keyboardMapper - MAC en_us', () => {
        let mapper;
        suiteSetup(async () => {
            const _mapper = await createKeyboardMapper(true, 'mac_en_us', 2 /* Macintosh */);
            mapper = _mapper;
        });
        test('mapping', () => {
    ...
    suite('keyboardMapper - LINUX de_ch', () => {
        let mapper;
        suiteSetup(async () => {
            const _mapper = await createKeyboardMapper(false, 'linux_de_ch', 3 /* Linux */);
            mapper = _mapper;
        });
        test('mapping', () => {
    ...
    suite('keyboardMapper - LINUX en_us', () => {
        let mapper;
        suiteSetup(async () => {
            const _mapper = await createKeyboardMapper(true, 'linux_en_us', 3 /* Linux */);
            mapper = _mapper;
        });
        test('mapping', () => {
    ...
    suite('keyboardMapper - LINUX ru', () => {
        let mapper;
        suiteSetup(async () => {
            const _mapper = await createKeyboardMapper(false, 'linux_ru', 3 /* Linux */);
            mapper = _mapper;
        });
        test('mapping', () => {
    ...
    suite('keyboardMapper - LINUX en_uk', () => {
        let mapper;
        suiteSetup(async () => {
            const _mapper = await createKeyboardMapper(false, 'linux_en_uk', 3 /* Linux */);
            mapper = _mapper;
        });
        test('mapping', () => {
    ...
    suite('keyboardMapper - MAC zh_hant', () => {
        let mapper;
        suiteSetup(async () => {
            const _mapper = await createKeyboardMapper(false, 'mac_zh_hant', 2 /* Macintosh */);
            mapper = _mapper;
        });
        test('mapping', () => {
    ...
    function _assertKeybindingTranslation(mapper, OS, kb, _expected) {
        ...
        const runtimeKeybinding = (0, keyCodes_1.createSimpleKeybinding)(kb, OS);
