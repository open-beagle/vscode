...
define(["require", "exports", "vs/nls!vs/workbench/services/dialogs/browser/simpleFileDialog", "vs/base/common/resources", "vs/base/common/objects", "vs/platform/files/common/files", "vs/platform/quickinput/common/quickInput", "vs/base/common/uri", "vs/base/common/platform", "vs/platform/dialogs/common/dialogs", "vs/platform/label/common/label", "vs/platform/workspace/common/workspace", "vs/platform/notification/common/notification", "vs/editor/common/services/modelService", "vs/editor/common/services/modeService", "vs/editor/common/services/getIconClasses", "vs/base/common/network", "vs/workbench/services/environment/common/environmentService", "vs/workbench/services/remote/common/remoteAgentService", "vs/platform/contextkey/common/contextkey", "vs/base/common/strings", "vs/platform/keybinding/common/keybinding", "vs/base/common/extpath", "vs/base/common/event", "vs/base/common/lifecycle", "vs/base/common/async", "vs/workbench/services/editor/common/editorService", "vs/base/common/labels", "vs/workbench/services/path/common/pathService", "vs/platform/accessibility/common/accessibility"], function (require, exports, nls, resources, objects, files_1, quickInput_1, uri_1, platform_1, dialogs_1, label_1, workspace_1, notification_1, modelService_1, modeService_1, getIconClasses_1, network_1, environmentService_1, remoteAgentService_1, contextkey_1, strings_1, keybinding_1, extpath_1, event_1, lifecycle_1, async_1, editorService_1, labels_1, pathService_1, accessibility_1) {
    "use strict";
    ...
    let SimpleFileDialog = class SimpleFileDialog {
        constructor(fileService, quickInputService, labelService, workspaceContextService, notificationService, fileDialogService, modelService, modeService, environmentService, remoteAgentService, pathService, keybindingService, contextKeyService, accessibilityService) {
        ...
        getOptions(options, isSave = false) {
            ...
            if ((this.scheme !== network_1.Schemas.file) && !this.fileService.canHandleResource(defaultUri)) {
                this.notificationService.info(nls.localize(4, null, defaultUri.toString()));
                return undefined;
            }
        ...
        remoteUriFrom(path) {
            ...
            const authority = uri.scheme === network_1.Schemas.file ? undefined : this.remoteAuthority;
        ...
        async pickResource(isSave = false) {
            ...
            return new Promise(async (resolve) => {
                ...
                if ((this.scheme !== network_1.Schemas.file) && this.options && this.options.availableFileSystems && (this.options.availableFileSystems.length > 1) && (this.options.availableFileSystems.indexOf(network_1.Schemas.file) > -1)) {
                    ...
                    this.filePickBox.customLabel = nls.localize(5, null);
                    let action;
                    if (isSave) {
        ...
        constructFullUserPath() {
            ...
            if ((0, strings_1.equalsIgnoreCase)(this.filePickBox.value.substr(0, this.userEnteredPathSegment.length), this.userEnteredPathSegment) && (0, strings_1.equalsIgnoreCase)(this.filePickBox.value.substr(0, currentFolderPath.length), currentFolderPath)) {
        ...
        async tryUpdateItems(value, valueUri) {
            if ((value.length > 0) && ((value[value.length - 1] === '~') || (value[0] === '~'))) {
                ...
                if (stat && stat.isDirectory && (resources.basename(valueUri) !== '.') && this.endsWithSlash(value)) {
                    ...
                    this.filePickBox.validationMessage = nls.localize(6, null);
                    ...
                    if (!resources.extUriIgnorePathCase.isEqual(resources.removeTrailingPathSeparator(this.currentFolder), inputUriDirname)
                        && (!/^[a-zA-Z]:$/.test(this.filePickBox.value) || !(0, strings_1.equalsIgnoreCase)(this.pathFromUri(this.currentFolder).substring(0, this.filePickBox.value.length), this.filePickBox.value))) {
        ...
        async validate(uri) {
            if (uri === undefined) {
                this.filePickBox.validationMessage = nls.localize(8, null);
            ...
            if (this.requiresTrailing) { // save
                if (stat && stat.isDirectory) {
                    // Can't do this
                    this.filePickBox.validationMessage = nls.localize(9, null);
                    return Promise.resolve(false);
                }
                else if (stat) {
                    // Replacing a file.
                    // Show a yes/no prompt
                    const message = nls.localize(10, null, resources.basename(uri));
                    return this.yesNoPrompt(uri, message);
                }
                else if (!((0, extpath_1.isValidBasename)(resources.basename(uri), await this.isWindowsOS()))) {
                    // Filename not allowed
                    this.filePickBox.validationMessage = nls.localize(11, null);
                    return Promise.resolve(false);
                }
                else if (!statDirname || !statDirname.isDirectory) {
                    // Folder to save in doesn't exist
                    this.filePickBox.validationMessage = nls.localize(12, null);
                    return Promise.resolve(false);
                }
            }
            else { // open
                if (!stat) {
                    ...
                    this.filePickBox.validationMessage = nls.localize(13, null);
                    return Promise.resolve(false);
                }
                else if (uri.path === '/' && (await this.isWindowsOS())) {
                    this.filePickBox.validationMessage = nls.localize(14, null);
                    return Promise.resolve(false);
                }
                else if (stat.isDirectory && !this.allowFolderSelection) {
                    // Folder selected when folder selection not permitted
                    this.filePickBox.validationMessage = nls.localize(15, null);
                    return Promise.resolve(false);
                }
                else if (!stat.isDirectory && !this.allowFileSelection) {
                    // File selected when file selection not permitted
                    this.filePickBox.validationMessage = nls.localize(16, null);
                    return Promise.resolve(false);
                }
            }
        ...
        pathFromUri(uri, endWithSeparator = false) {
            let result = (0, labels_1.normalizeDriveLetter)(uri.fsPath).replace(/\n/g, '');
            if (this.separator === '/') {
        ...
        async createItem(stat, parent, token) {
            ...
            if (stat.isDirectory) {
                ...
                return { label: filename, uri: fullPath, isFolder: true, iconClasses: (0, getIconClasses_1.getIconClasses)(this.modelService, this.modeService, fullPath || undefined, files_1.FileKind.FOLDER) };
            }
            else if (!stat.isDirectory && this.allowFileSelection && this.filterFile(fullPath)) {
                return { label: stat.name, uri: fullPath, isFolder: false, iconClasses: (0, getIconClasses_1.getIconClasses)(this.modelService, this.modeService, fullPath || undefined) };
            }
            return undefined;
        }
    };
