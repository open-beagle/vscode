 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/workbench/services/textfile/common/textFileEditorModel", "vs/base/test/common/utils", "vs/workbench/test/browser/workbenchTestServices", "vs/workbench/test/common/workbenchTestServices", "vs/base/common/buffer", "vs/base/common/cancellation", "vs/base/common/async"], function (require, exports, assert, textFileEditorModel_1, utils_1, workbenchTestServices_1, workbenchTestServices_2, buffer_1, cancellation_1, async_1) {
    "use strict";
        ...
        test('createFolder', async function () {
            ...
            const participant = accessor.workingCopyFileService.addFileOperationParticipant({
                participate: async (files, operation) => {
                    ...
                    assert.strictEqual(operation, 0 /* CREATE */);
                    eventCounter++;
                }
            });
        ...
        async function testMoveOrCopy(files, move, targetDirty) {
            ...
            const participant = accessor.workingCopyFileService.addFileOperationParticipant({
                participate: async (files, operation) => {
                    ...
                    eventCounter++;
                    assert.strictEqual(operation, move ? 2 /* MOVE */ : 3 /* COPY */);
                }
            });
            let correlationId;
            const listener1 = accessor.workingCopyFileService.onWillRunWorkingCopyFileOperation(e => {
                ...
                eventCounter++;
                correlationId = e.correlationId;
                assert.strictEqual(e.operation, move ? 2 /* MOVE */ : 3 /* COPY */);
            });
            const listener2 = accessor.workingCopyFileService.onDidRunWorkingCopyFileOperation(e => {
                for (let i = 0; i < e.files.length; i++) {
                    const { target, source } = files[i];
        ...
        async function testDelete(resources) {
            ...
            const participant = accessor.workingCopyFileService.addFileOperationParticipant({
                participate: async (files, operation) => {
                    ...
                    assert.strictEqual(operation, 1 /* DELETE */);
                    eventCounter++;
                }
            });
            const listener1 = accessor.workingCopyFileService.onWillRunWorkingCopyFileOperation(e => {
                ...
                assert.strictEqual(e.operation, 1 /* DELETE */);
                correlationId = e.correlationId;
                eventCounter++;
            });
            const listener2 = accessor.workingCopyFileService.onDidRunWorkingCopyFileOperation(e => {
