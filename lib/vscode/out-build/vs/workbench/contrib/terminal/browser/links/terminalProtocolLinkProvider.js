    ...
    let TerminalProtocolLinkProvider = class TerminalProtocolLinkProvider extends terminalBaseLinkProvider_1.TerminalBaseLinkProvider {
        ...
        _provideLinks(y) {
            ...
            return links.map(link => {
                var _a;
                const range = (0, terminalLinkHelpers_1.convertLinkRangeToBuffer)(lines, this._xterm.cols, link.range, startLine);
                // Check if the link if within the mouse position
                const uri = link.url
                    ? (typeof link.url === 'string' ? uri_1.URI.parse(link.url) : link.url)
                    : undefined;
                const label = ((uri === null || uri === void 0 ? void 0 : uri.scheme) === network_1.Schemas.file) ? terminalLink_1.OPEN_FILE_LABEL : undefined;
                return this._instantiationService.createInstance(terminalLink_1.TerminalLink, this._xterm, range, ((_a = link.url) === null || _a === void 0 ? void 0 : _a.toString()) || '', this._xterm.buffer.active.viewportY, this._activateCallback, this._tooltipCallback, true, label);
            });
        }
    };
