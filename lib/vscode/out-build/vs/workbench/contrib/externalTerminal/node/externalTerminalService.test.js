 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/workbench/contrib/externalTerminal/node/externalTerminalService", "vs/workbench/contrib/externalTerminal/node/externalTerminal"], function (require, exports, assert_1, externalTerminalService_1, externalTerminal_1) {
    "use strict";
    ...
    suite('ExternalTerminalService', () => {
        ...
        test(`WinTerminalService - uses terminal from configuration`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    (0, assert_1.equal)(command, testShell, 'shell should equal expected');
                    (0, assert_1.equal)(args[args.length - 1], mockConfig.terminal.external.windowsExec, 'terminal should equal expected');
                    (0, assert_1.equal)(opts.cwd, testCwd, 'opts.cwd should equal expected');
                    done();
                    return {
                        on: (evt) => evt
                    };
        ...
        test(`WinTerminalService - uses default terminal when configuration.terminal.external.windowsExec is undefined`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    (0, assert_1.equal)(args[args.length - 1], externalTerminalService_1.WindowsExternalTerminalService.getDefaultTerminalWindows(), 'terminal should equal expected');
                    done();
                    return {
                        on: (evt) => evt
                    };
        ...
        test(`WinTerminalService - uses default terminal when configuration.terminal.external.windowsExec is undefined`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    (0, assert_1.equal)(opts.cwd, 'C:/foo', 'cwd should be uppercase regardless of the case that\'s passed in');
                    done();
                    return {
                        on: (evt) => evt
                    };
        ...
        test(`WinTerminalService - cmder should be spawned differently`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    (0, assert_1.deepEqual)(args, ['C:/foo']);
                    (0, assert_1.equal)(opts, undefined);
                    done();
                    return { on: (evt) => evt };
                }
            };
        ...
        test(`WinTerminalService - windows terminal should open workspace directory`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    (0, assert_1.equal)(opts.cwd, 'C:/foo');
                    done();
                    return { on: (evt) => evt };
                }
            };
        ...
        test(`MacTerminalService - uses terminal from configuration`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    (0, assert_1.equal)(args[1], mockConfig.terminal.external.osxExec, 'terminal should equal expected');
                    done();
                    return {
                        on: (evt) => evt
                    };
        ...
        test(`MacTerminalService - uses default terminal when configuration.terminal.external.osxExec is undefined`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    (0, assert_1.equal)(args[1], externalTerminal_1.DEFAULT_TERMINAL_OSX, 'terminal should equal expected');
                    done();
                    return {
                        on: (evt) => evt
                    };
        ...
        test(`LinuxTerminalService - uses terminal from configuration`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    (0, assert_1.equal)(command, mockConfig.terminal.external.linuxExec, 'terminal should equal expected');
                    (0, assert_1.equal)(opts.cwd, testCwd, 'opts.cwd should equal expected');
                    done();
                    return {
                        on: (evt) => evt
        ...
        test(`LinuxTerminalService - uses default terminal when configuration.terminal.external.linuxExec is undefined`, done => {
            externalTerminalService_1.LinuxExternalTerminalService.getDefaultTerminalLinuxReady().then(defaultTerminalLinux => {
                ...
                let mockSpawner = {
                    spawn: (command, args, opts) => {
                        // assert
                        (0, assert_1.equal)(command, defaultTerminalLinux, 'terminal should equal expected');
                        done();
                        return {
                            on: (evt) => evt
                        };
